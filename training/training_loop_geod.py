...
def training_loop(
    ...
    IR_kwargs               = {},       # Options for inverse renderer network.
    G_opt_kwargs            = {},       # Options for generator optimizer.
    D_opt_kwargs            = {},       # Options for discriminator optimizer.
    IR_opt_kwargs           = {},       # Options for inverse renderer optimizer.
    ...
    dis_pose_cond           = True,     # Whether use pose conditioning in the discriminator.
    IR_stop                 = 5000000,  # GeoD-related, not used
    IR_path                 = None      # GeoD-related, not used
):
    # Initialize.
    ...
    G = dnnlib.util.construct_class_by_name(c_dim=training_set.label_dim, **G_kwargs, **common_kwargs).train().requires_grad_(False).to(device) # subclass of torch.nn.Module
    ...
    IR = dnnlib.util.construct_class_by_name(device=device, **IR_kwargs).train().requires_grad_(False).to(device) # subclass of torch.nn.Module
    ...
    if (resume_pkl is not None) and (rank == 0):
        ...
        for name, module in [('G', G), ('D', D), ('IR', IR), ('G_ema', G_ema)]:
            misc.copy_params_and_buffers(resume_data[name], module, require_all=False)
    
    if (IR_path is not None) and (rank == 0):
        print(f'Resuming GeoD from "{IR_path}"')
        with dnnlib.util.open_url(IR_path) as f:
            resume_IR = legacy.load_network_pkl(f)
        misc.copy_params_and_buffers(resume_IR['IR'], IR, require_all=False)
    ...
    for module in [IR, G, D, G_ema, augment_pipe]:
    ...
    loss = dnnlib.util.construct_class_by_name(device=device, G=G, D=D, IR=IR, augment_pipe=augment_pipe, **loss_kwargs) # subclass of training.loss.Loss
    phases = []
    for name, module, opt_kwargs, reg_interval in [('IR', IR, IR_opt_kwargs, IR_reg_interval), ('G', G, G_opt_kwargs, G_reg_interval), ('D', D, D_opt_kwargs, D_reg_interval)]:
    ...
    while True:
        ...
        if (cur_nimg > IR_stop * 1000) or (IR_path is not None):
            for phase in phases:
                if phase.name == 'IRboth':
                    phases.remove(phase)
                    break
        ...
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
            ...
            for z, c, img in zip(grid_z, grid_c, grid_i):
                results = G_ema(z=z, c=c, noise_mode='const')
                if results['image'].size(2) != 64:
                    scale_factor = 64 / results['image'].size(2)
                    input_img = F.interpolate(results['image'], scale_factor=scale_factor, mode='bilinear')
                else:
                    scale_factor = 1
                    input_img = results['image']
                ir_normal = IR(input_img)['recon_normal'][:input_img.size(0)].permute(0,3,1,2)
                if scale_factor != 1:
                    ir_normal = F.interpolate(ir_normal, scale_factor=1/scale_factor, mode='bilinear')

                if img.size(2) != 64:
                    scale_factor = 64 / img.size(2)
                    real_input = F.interpolate(img, scale_factor=scale_factor, mode='bilinear')
                else:
                    scale_factor = 1
                    real_input = img
                ir_normal_real = IR(real_input)['recon_normal'][:real_input.size(0)].permute(0,3,1,2)
                if scale_factor != 1:
                    ir_normal_real = F.interpolate(ir_normal_real, scale_factor=1/scale_factor, mode='bilinear')
                
                images.append(results['image'].cpu())
                images_raw.append(results['image_raw'].cpu())
                images_depth.append(results['image_depth'].cpu())
                images_normal.append(results['normal_map'].cpu())
                ir_normal_list.append(ir_normal.cpu())
                ir_normal_real_list.append(ir_normal_real.cpu())
                    


            images = torch.cat(images).numpy()
            images_raw = torch.cat(images_raw).numpy()
            images_depth = -torch.cat(images_depth).numpy()
            images_normal = torch.cat(images_normal).numpy()
            ir_normal_list = torch.cat(ir_normal_list).numpy()
            ir_normal_real_list = torch.cat(ir_normal_real_list).numpy()
            ...
            save_image_grid(ir_normal_list, os.path.join(run_dir, f'fakes{cur_nimg//1000:06d}_normal_ir.png'), drange=[-1,1], grid_size=grid_size)
            save_image_grid(ir_normal_real_list, os.path.join(run_dir, f'fakes{cur_nimg//1000:06d}_normal_ir_real.png'), drange=[-1,1], grid_size=grid_size)
        ...
        if (network_snapshot_ticks is not None) and (done or cur_tick % network_snapshot_ticks == 0):
            ...
            for name, module in [('IR', IR), ('G', G), ('D', D), ('G_ema', G_ema), ('augment_pipe', augment_pipe)]:
                if name == 'IR':
                    module = dnnlib.util.construct_class_by_name(**IR_kwargs).train().requires_grad_(False).cpu() # subclass of torch.nn.Module
                    for p_ema, p in zip(module.parameters(), IR.parameters()):
                        p_ema.copy_(p.data)
                    for b_ema, b in zip(module.buffers(), IR.buffers()):
                        b_ema.copy_(b.data)
