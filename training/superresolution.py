...
class CNN3x3(torch.nn.Module):
    def __init__(self, channels, img_resolution, sr_num_fp16_res, sr_antialias,
        ...
        self.sr_antialias = sr_antialias
        self.block0 = SynthesisBlockNoUp(channels, 128, w_dim=512, resolution=img_resolution,
                img_channels=3, is_last=False, use_fp16=use_fp16, conv_clamp=(256 if use_fp16 else None), **block_kwargs)
        self.block1 = SynthesisBlockNoUp(128, 64, w_dim=512, resolution=img_resolution,
                img_channels=3, is_last=True, use_fp16=use_fp16, conv_clamp=(256 if use_fp16 else None), **block_kwargs)
        self.register_buffer('resample_filter', upfirdn2d.setup_filter([1,3,3,1]))
...
class CNN1x1(torch.nn.Module):
    def __init__(self, channels, img_resolution, sr_num_fp16_res, sr_antialias,
        ...
        self.input_resolution = img_resolution
        self.sr_antialias = sr_antialias
        self.block0 = SynthesisBlockNoUp1x1(channels, 64, w_dim=512, resolution=img_resolution,
                img_channels=3, is_last=True, use_fp16=use_fp16, conv_clamp=(256 if use_fp16 else None), **block_kwargs)
        # self.block0 = SynthesisBlockNoUp1x1(channels, 128, w_dim=512, resolution=img_resolution,
        #         img_channels=3, is_last=False, use_fp16=use_fp16, conv_clamp=(256 if use_fp16 else None), **block_kwargs)
        # self.block1 = SynthesisBlockNoUp1x1(128, 64, w_dim=512, resolution=img_resolution,
        #         img_channels=3, is_last=True, use_fp16=use_fp16, conv_clamp=(256 if use_fp16 else None), **block_kwargs)
        self.register_buffer('resample_filter', upfirdn2d.setup_filter([1,3,3,1]))
...
@persistence.persistent_class
class SynthesisBlockNoUp1x1(torch.nn.Module):
    def __init__(self,
        in_channels,                            # Number of input channels, 0 = first block.
        out_channels,                           # Number of output channels.
        w_dim,                                  # Intermediate latent (W) dimensionality.
        ...
        **layer_kwargs,                         # Arguments for SynthesisLayer.
        ...
        if in_channels != 0:
            self.conv0 = SynthesisLayer(in_channels, out_channels, w_dim=w_dim, resolution=resolution,
                conv_clamp=conv_clamp, channels_last=self.channels_last, kernel_size=1, **layer_kwargs)
            self.num_conv += 1
        ...
        if is_last or architecture == 'skip':
            self.torgb = ToRGBLayer(out_channels, img_channels, w_dim=w_dim,
                conv_clamp=conv_clamp, channels_last=self.channels_last, kernel_size=1)
            self.num_torgb += 1
